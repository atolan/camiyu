                        ...
                        for($year = date("Y"); $year >= $start; $year--) { 
                            if($y != $year) {?>
                                <div class="radio_contain">
                                    <input id="<?php echo $year; ?>-sp" value="<?php echo $year; ?>" type="radio" name="year" class="radio" onclick='window.location.href="<?php echo get_post_type_archive_link("news"). '?y=' . $year; ?>"'>
...
<!--      Year Smartphone Menu      -->
        ...
        <section class="post-content inner">
            <div class="post-list">
                ...
                <?php
                    $post_args = array( 'post_type' => 'news', 'orderby' => 'menu_order', 'order' => 'ASC', 'posts_per_page' => 5 );
                    $y  = $_GET['y'];
                    if (!empty($y))
                        $post_args = array( 'post_type' => 'news', 'posts_per_page' => 5, 'orderby' => 'menu_order', 'order' => 'ASC', 'date_query' => array(
                            'relation' => 'AND',
                            array('year' => $y)
                        ) );
