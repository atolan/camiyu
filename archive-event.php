...
?>
    ...
    <div class="main-content">
                        ...
                        $start = 2009;
                            ...
                            if($y != $year) {?>
                                <div class="radio_contain">
                                    <input id="<?php echo $year; ?>-sp" value="<?php echo $year; ?>" type="radio" name="year" class="radio" onclick='window.location.href="<?php echo get_post_type_archive_link("event"). '?y=' . $year; ?>"'>
                                    <label for="<?php echo $year; ?>-sp"><?php echo $year; ?>年</label>
                                </div>
                            <?php } 
                            else { ?>
                                <div class="radio_contain">
                                    <input id="<?php echo $year; ?>-sp" value="<?php echo $year; ?>" type="radio" name="year" class="radio" onclick='window.location.href="<?php echo get_post_type_archive_link("event"). '?y=' . $year; ?>"' checked>
                                    <label for="<?php echo $year; ?>-sp"><?php echo $year; ?>年</label>
                                </div>
                    <?php } } ?>
                </div>
        ...
        <section class="post-content inner">
            <div class="post-list">
                ...
                <?php
                    ...
                    <?php }
                    $post_args = array( 'post_type' => 'event', 'orderby' => 'menu_order', 'order' => 'ASC', 'posts_per_page' => 5 );
                    $y  = $_GET['y'];
                    if (!empty($y))
                        $post_args = array( 'post_type' => 'event', 'orderby' => 'menu_order', 'order' => 'ASC', 'posts_per_page' => 5, 'date_query' => array(
                            'relation' => 'AND',
                            array('year' => $y)
                        ) );
                    ...
                    <div class="radio-group">
                        <?php
                            ...
                            for($year = date("Y"); $year >= $start; $year--) {  
                                if($y != $year) {?>
                                    <div class="radio_contain">
                                        <input id="<?php echo $year; ?>" value="<?php echo $year; ?>" type="radio" name="year" class="radio" onclick='window.location.href="<?php echo get_post_type_archive_link("event"). '?y=' . $year; ?>"'>
                                        <label for="<?php echo $year; ?>"><?php echo $year; ?>年</label>
                                    </div>
                                <?php } 
                                else { ?>
                                    <div class="radio_contain">
                                        <input id="<?php echo $year; ?>" value="<?php echo $year; ?>" type="radio" name="year" class="radio" onclick='window.location.href="<?php echo get_post_type_archive_link("event"). '?y=' . $year; ?>"' checked>
                                        <label for="<?php echo $year; ?>"><?php echo $year; ?>年</label>
                                    </div>
                            <?php } } ?>
                    </div>
        ...
        <section class="inquery">
            ...
            <button class="inquery-btn" data-aos="fade-up" data-aos-delay="600">
                お問い合わせフォーム
